...
void save_density_pgm_old(const float *outputPtr, int W, int H, const char *filename) {
...
void save_density_pgm(const float *outputPtr, int W, int H, const char *filename) {
    ...
    for (int y = 0; y < H; y++) {
        for (int x = 0; x < W; x++) {
            ...
            if (norm < 0) norm = 0;
            if (norm > 1) norm = 1;
            unsigned char gray = (unsigned char)(norm);
            fputc(gray, f);
        }
    }
    fclose(f);
...
void save_density_raw(const float *outputPtr, int W, int H, const char *filename) {
    FILE *f = fopen(filename, "wb");
    if (!f) {
        fprintf(stderr, "Cannot open file %s\n", filename);
        return;
    }
    fwrite(outputPtr, sizeof(float), W * H, f);
    fclose(f);
    printf("Saved raw float array to %s\n", filename);
}
...
void create_model1() {
    ...
    layer = model1->lastLayer;
    save_density_raw(layer->output, 128, 128, "heapmap/density_map.raw");

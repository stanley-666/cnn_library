                  ...
                  ActivationType activation, int inputWidth, int inputHeight, int inputChannels)
{
    ...
    Layer *layer = (Layer *)safeMalloc("layer", sizeof(Layer));
    layer->type = CONV;
...
void addPoolLayer(CNN *net, int poolSize, int stride, PoolType poolType, int inputWidth, int inputHeight, int inputChannels)
{
    ...
    layer->output = (float *)safeCalloc("layer->output", outputSize, sizeof(float));
...
void addFCLayer(CNN *net, int outputSize, ActivationType activation, int inputSize)
{
    ...
    // 分配權重和偏置空間
    layer->params.fc.weights = (float *)safeCalloc( "Weight",inputSize * outputSize, sizeof(float));
    layer->params.fc.bias = (float *)safeCalloc("bias", outputSize, sizeof(float));
...
{   
    ...
    int entries = paddedW * paddedH * inC;
    // padding = 1 矩陣外維繞一圈補0
    if(padding == 0) {
        paddedInput = input;
        allocated_size = 0;
        ...
        allocated_size = entries * sizeof(float);
        paddedInput = (float *)safeCalloc("paddedInput", entries, sizeof(float));
    ...
    layer->output = (float *)safeCalloc("layer->output", outC * outH * outW, sizeof(float));
    ...
    for (int oc = 0; oc < outC; oc++) {
        for (int oh = 0; oh < outH; oh++) {
            for (int ow = 0; ow < outW; ow++) {
                ...
                if (layerCount == 0) {
                    layer->output[(oc * outH + oh) * outW + ow] = temp*0.0010804759804159403;
                }           
                else if(layerCount == 1) {
                    layer->output[(oc * outH + oh) * outW + ow] = temp*0.0025358216371387243;                }
                else if(layerCount == 2) {
                    layer->output[(oc * outH + oh) * outW + ow] = temp*0.009047127328813076;
                }
                else if(layerCount == 3) {
                    layer->output[(oc * outH + oh) * outW + ow] = temp*0.009172260761260986;
                }else if(layerCount == 4) {
                    layer->output[(oc * outH + oh) * outW + ow] = temp*0.006259026471525431;
                }else if(layerCount == 5) {
                    layer->output[(oc * outH + oh) * outW + ow] = temp*0.019057000055909157;
                }else if(layerCount == 6) {
                    layer->output[(oc * outH + oh) * outW + ow] = temp*0.07177232950925827;
                }else if(layerCount == 7) {
                    layer->output[(oc * outH + oh) * outW + ow] = temp*0.013572472147643566;
                }else if(layerCount == 8) {
                    layer->output[(oc * outH + oh) * outW + ow] = temp*0.04157446324825287;
                }else if(layerCount == 9) {
                    layer->output[(oc * outH + oh) * outW + ow] = temp*0.010972687974572182;
                }
                else {
    ...
    if( padding > 0 ) {
        free(paddedInput);
        fprintf(stderr,"Free padding memory size: %lu bytes\n", allocated_size);
        g_total_allocated -=  allocated_size;
    }
    if ( layerCount == 10) {

...
void backwardFC(Layer* layer, float* target) {
    ...
    if (!layer->deltas) 
        layer->deltas = (float*)safeCalloc("layer->deltas", outSize, sizeof(float));
    if (!layer->params.fc.weights || !layer->params.fc.bias) {
        printf("[ERROR] FC layer missing weights/bias!\n");
        exit(EXIT_FAILURE);
    }
    if (!layer->weightGrads) 
        layer->weightGrads = (float*)safeCalloc("layer->weightGrads", inSize * outSize, sizeof(float));
    if (!layer->biasGrads)
        layer->biasGrads = (float*)safeCalloc("layer->biasGrads", outSize, sizeof(float));
...
void backwardPool(Layer* layer) {
    ...
    if (!layer->deltas) layer->deltas = (float*)safeCalloc("layer->deltas", inW * inH * inC, sizeof(float));
    // update deltas
    for (int c = 0; c < inC; c++) {
...
void backwardConv(Layer* layer) {
    ...
    if (!layer->deltas)
        layer->deltas = (float*)safeCalloc("layer->deltas", inW * inH * inC, sizeof(float));
    if (!layer->weightGrads)
        layer->weightGrads = (float*)safeCalloc("layer->weightGrads", filterSize * filterSize * inC * outC, sizeof(float));
    if (!layer->biasGrads)
        layer->biasGrads = (float*)safeCalloc("layer->biasGrads", outC, sizeof(float));
